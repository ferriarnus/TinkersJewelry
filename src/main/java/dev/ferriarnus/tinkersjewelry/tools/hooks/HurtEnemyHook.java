...
public interface HurtEnemyHook {
    void hurtEnemy(ItemStack stack, DamageSource source, double damage, @Nullable LivingEntity defender, @Nullable LivingEntity attacker);
    ...
    record AllMerger(Collection<HurtEnemyHook> modules) implements HurtEnemyHook {
        ...
        public void hurtEnemy(ItemStack stack, DamageSource source, double damage, @Nullable LivingEntity defender, @Nullable LivingEntity attacker) {
            for (HurtEnemyHook hook : modules) {
                hook.hurtEnemy(stack, source, damage, defender, attacker);
            }
        }
    }
}